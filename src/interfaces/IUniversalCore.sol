...
interface IUniversalCore {
    ...
    function setWrapperDeployed(address sourceAsset, string calldata destChain, bytes32 wrapper) external;
    ...
    function updatePC20FactoryByChain(string memory chainNamespace, bytes32 factory) external;

    /// @notice                  Get the PC20Factory identity for a chain.
    /// @param chainNamespace    Chain namespace
    /// @return factory          PC20Factory identity (bytes32)
    function pc20FactoryByChain(string memory chainNamespace) external view returns (bytes32 factory);

    /// @notice                  Get the wrapper identity for a source asset on a chain.
    /// @param sourceAsset       Source asset address
    /// @param destChain         Destination chain namespace
    /// @return wrapper          Wrapper identity (bytes32)
    function pc20WrapperBySource(address sourceAsset, string memory destChain) external view returns (bytes32 wrapper);
