...
interface IHandler {
    ...
    event GasFundedWithGasToken(bytes32 payloadId, uint256 dstChainId, address gasToken, uint256 amount, address from, address to);
    event GasFundedViaSwap(bytes32 payloadId, uint256 dstChainId, address tokenIn, uint256 amountIn, address gasToken, uint256 amountOut, uint24 fee, address from, address to);
}
