...
the `VAULT` address set at initialization. `isExecuted[subTxId]` provides per-CEA
...
| `executeUniversalTx(subTxId, ...)`             | `VAULT`                 | `onlyVault`, `nonReentrant`, `payable`                  |
...
| CEA-T6 | Tampering              | `isExecuted[subTxId] = true` is set before `_handleExecution`; a revert unwinds the entire transaction including the flag — replay protection is transaction-atomic (safe), but auditors should confirm this covers all execution paths                                                   |
...
1. `isExecuted[subTxId]` transitions only `false → true`, never reset
