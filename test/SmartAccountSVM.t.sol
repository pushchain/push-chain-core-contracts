...
contract SmartAccountSVMTest is Test {
    ...
    function testAccountId() public deploySvmSmartAccount {
        ...
        assertEq(uint(accountId.vmType), uint(VM_TYPE.SVM));
    }
    ...
    function testDeploymentCreate2() public deploySvmSmartAccount {
        ...
        assertEq(
            ...
            address(factory.userAccounts(ownerKey))
        );
        
        assertEq(
            address(svmSmartAccountInstance),
            address(factory.computeSmartAccountAddress(_owner))
        );
    }
    
    function testExecution() public deploySvmSmartAccount {
        // Prepare calldata for target contract
        uint previousNonce = svmSmartAccountInstance.nonce();
    ...
    function getCrosschainTxhash(
        SmartAccountSVM _smartAccountInstance,
