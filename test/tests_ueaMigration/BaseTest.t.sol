...
contract BaseTest is Test {
    ...
    function _deployMigrationContract() internal {
        migration = new UEAMigration(
            address(ueaEVMImplV2),
            address(ueaSVMImplV2)
        );
        
        assertEq(
            migration.UEA_EVM_IMPLEMENTATION(), 
            address(ueaEVMImplV2), 
            "Migration EVM implementation mismatch"
        );
        assertEq(
            migration.UEA_SVM_IMPLEMENTATION(), 
            address(ueaSVMImplV2), 
            "Migration SVM implementation mismatch"
        );
    }
    ...
    function _createTestAccountIds() internal {
        evmAccountId = UniversalAccountId({
        ...
        svmAccountId = UniversalAccountId({
            chainNamespace: SVM_CHAIN_NAMESPACE,
    ...
    function createEVMMigrationPayload(uint256 nonce, uint256 deadline) 
        public 
        view 
        returns (MigrationPayload memory) 
    {
    ...
    function createSVMMigrationPayload(uint256 nonce, uint256 deadline) 
        public 
        view 
        returns (MigrationPayload memory) 
    {
    ...
    function signEVMMigrationPayload(
        UEA_EVM ueaInstance,
    ...
    function assertStatePreserved(
        address ueaProxy,
        UniversalAccountId memory expectedAccount,
    ...
    function testMigrationContractSetup() public {
        ...
        assertEq(
            migration.UEA_EVM_IMPLEMENTATION(), 
            address(ueaEVMImplV2),
            "Migration EVM implementation incorrect"
        );
        assertEq(
            migration.UEA_SVM_IMPLEMENTATION(), 
            address(ueaSVMImplV2),
            "Migration SVM implementation incorrect"
        );
